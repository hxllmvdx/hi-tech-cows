...
sudo apt update
sudo apt install -y python3-dev python3.11-dev libatlas-base-dev libopenblas-dev liblapack-dev gfortran \
...
if [ ! -d "cow_counter_env" ]; then
    uv venv cow_counter_env --system-packages
fi
...
# Шаг 8: Проверка установки Coral TPU
echo "✅ Шаг 8: Проверка установки Coral TPU"
if lsusb | grep -
